...
public class EvaluatorOptimizer {
     ...
     */
    public Records.RefinedResponse loop(String task) {
        List<String> memory = new ArrayList<>();
        List<Records.Generation> chainOfThought = new ArrayList<>();
     ...
     * history
     */
    private Records.RefinedResponse loop(String task, String context, List<String> memory, List<Records.Generation> chainOfThought) {

        Records.Generation generation = this.generate(task, context);
        memory.add(generation.response());
        chainOfThought.add(generation);

        Records.EvaluationResponse evaluationResponse = this.evaluate(generation.response(), task);

        if (evaluationResponse.evaluation().equals(Records.EvaluationResponse.Evaluation.PASS)) {
            // Solution is accepted!
            return new Records.RefinedResponse(generation.response(), chainOfThought);
        }
        ...
        return this.loop(task, newContext.toString(), memory, chainOfThought);
    }
     ...
     */
    private Records.Generation generate(String task, String context) {
        Records.Generation generationResponse = chatClient.prompt()
                .user(u -> u.text("{prompt}\n{context}\nTask: {task}")
                        .param("prompt", this.generatorPrompt)
                        .param("context", context)
                        .param("task", task))
                .call()
                .entity(Records.Generation.class);
     ...
     */
    private Records.EvaluationResponse evaluate(String content, String task) {

        Records.EvaluationResponse evaluationResponse = chatClient.prompt()
                .user(u -> u.text("{prompt}\nOriginal task: {task}\nContent to evaluate: {content}")
                        .param("prompt", this.evaluatorPrompt)
                        .param("task", task)
                        .param("content", content))
                .call()
                .entity(Records.EvaluationResponse.class);
